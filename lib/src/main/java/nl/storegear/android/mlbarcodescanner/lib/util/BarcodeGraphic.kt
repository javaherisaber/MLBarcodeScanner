...
import nl.storegear.android.mlbarcodescanner.mlkit.GraphicOverlay
...
class BarcodeGraphic constructor(
    ...
    override fun draw(canvas: Canvas) {
        ...
        val focusBoxSize = MetricUtils.dpToPx(264).toFloat()
        val left = (width.toFloat() / 2) - (focusBoxSize / 2)
        val right = (width.toFloat() / 2) + (focusBoxSize / 2)
        val top = (height / 2) - (focusBoxSize / 2)
        val bottom = (height / 2) + (focusBoxSize / 2)
        val focusArea = RectF(left, top, right, bottom)

        fun barcodeIsInFocusArea(focus: RectF, target: RectF): Boolean {
            return (focus.left < target.left) && (target.right < focus.right)
                    && (focus.top < target.top) && (focus.bottom > target.bottom)
        }
        val isInFocusArea = barcodeIsInFocusArea(focusArea, rect)
